...
Dev17/v0.2.2 is the Nerve launch. Nerve is the canonical product and Hermes plugin identity. The Python distribution and GitHub repository use the `hermes-nerve` slug, while Jev, Laya, and OpenJev remain backend names only where they refer to those Reflex implementations.
