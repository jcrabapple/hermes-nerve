# Nerve 0.2.1 bug-handoff actualization
...
| JEV-002 | Fixed | Stable `decision_id` values link decision rows to control lifecycle rows (`created`, `delivered`, `next_action`, `expired`) and outcomes. `nerve_stats` exposes attribution counts and an explicit decision-correction denominator definition. |
...
| JEV-004 | Fixed | `nerve_stats` defaults to a bounded `summary`, supports section filters, makes recent arrays opt-in, caps recent limits, and defensively truncates oversized targeted output. |
...
| JEV-008 | Plugin-side improvement | Plugin registration logs exact Nerve version and source path. The package does not claim to eliminate Hermes host warnings emitted before general-plugin discovery; those remain an upstream integration concern if reproduced. |
...
`nerve_stats` labels metric provenance instead of merging unlike lifetimes:
