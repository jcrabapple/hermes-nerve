...
Nerve supports OpenRouter Decisions, direct TypeSafe System One, and OpenCode Zen System One. Only the selected provider credential is required.

## Proven live paths

### OpenRouter -> TypeSafe

A real Muna/Hermes session invoked the registered `nerve_decide`, `nerve_assess`, and `nerve_context_curate` tools through OpenRouter and received TypeSafe provider metadata, model/request IDs, usage/cost, and network latency. This proves the Hermes -> plugin -> OpenRouter -> TypeSafe path rather than main-model simulation.

### Direct TypeSafe System One

Issue #1 contains two independent direct-account reports against Nerve v0.2.1.1. One reported a successful smoke with `live_provider_call: true`, `transport: typesafe-system-one`, `provenance_status: VERIFIED`, provider `TypeSafe`, model `jev-1.13.0`, a TypeSafe request ID, a receipt ID, `ok: true`, and approximately 299 ms latency. A second tester ran the smoke, full live suite, and the plugin through Hermes itself: all eight tools registered, the direct transport remained verified, and the suite completed successfully.
...
```bash
export HERMES_NERVE_PROVIDER=openrouter
...
```bash
export HERMES_NERVE_PROVIDER=typesafe
...
```bash
export HERMES_NERVE_PROVIDER=opencode
export OPENCODE_API_KEY='...'
export HERMES_NERVE_OPENCODE_MODEL=jev-1.13
python3 scripts/live_api_smoke.py
```
