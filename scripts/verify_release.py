...
def catalog_version():
 text=(ROOT/"packaging/hermes-catalog/jev.yaml").read_text();m=re.search(r'^version:\s*["\']?([^"\'\s]+)',text,re.M);assert m;return m.group(1)
...
def main():
 ...
 assert catalog_version()!=EXPECTED and ".dev" not in catalog_version()
 ...
 for rel in ("hermes_jev/reflex/laya.py","hermes_jev/reflex/openjev.py","hermes_jev/reflex/shadow.py","hermes_jev/reflex/laya_service.py","tests/test_reflex_laya_dev15b.py","tests/test_reflex_openjev_dev17.py","tests/test_dev15_controller_completion.py","docs/DEV15B_LAYA_INTEGRATION.md","docs/DEV15_INSTALL_AND_RETEST.md","docs/DEV16_INSTALL_AND_RETEST.md","docs/DEV17_OPEN_SOURCE_VALIDATION.md","scripts/install_dev15_profile.sh","scripts/verify_dev15.sh","scripts/install_dev16_profile.sh","scripts/verify_dev16.sh","scripts/install_dev17_profile.sh","scripts/verify_dev17.sh","scripts/check_laya_sidecar.py","scripts/check_openjev_sidecar.py","scripts/configure_reflex_profile.py","scripts/setup_laya_dev17.sh","scripts/setup_openjev_dev17.sh","scripts/run_dev17_model_matrix.py","tests/test_dev17_matrix_qol.py","tests/test_dev16_nerve_budget.py","hermes_jev/work/nerve.py","scripts/benchmark_preflight.py","docs/DEV16_SESSION_FINDINGS.md"):
 ...
 with tempfile.TemporaryDirectory() as t:
  td=Path(t);spec=importlib.util.spec_from_file_location("hermes_jev_plugin_verify",ROOT/"__init__.py",submodule_search_locations=[str(ROOT)]);mod=importlib.util.module_from_spec(spec);sys.modules[spec.name]=mod;spec.loader.exec_module(mod);ctx=Ctx(td);mod.register(ctx)
  ...
  old_env={k:os.environ.get(k) for k in ("HERMES_KANBAN_TASK","HERMES_KANBAN_RUN_ID","HERMES_KANBAN_CLAIM_LOCK","HERMES_JEV_OFFLINE_VERIFY")}
  try:
   os.environ["HERMES_KANBAN_TASK"]="verify-headless";os.environ["HERMES_KANBAN_RUN_ID"]="1";os.environ["HERMES_KANBAN_CLAIM_LOCK"]="claim";os.environ["HERMES_JEV_OFFLINE_VERIFY"]="1"
 ...
 from hermes_jev import schemas
 variants=schemas.JEV_ASSESS["parameters"]["properties"]["questions"]["additionalProperties"]["oneOf"];by={v["properties"]["type"]["enum"][0]:v for v in variants};assert by["choice"]["properties"]["criteria"]["minProperties"]==2;assert by["score"]["properties"]["criteria"]["minItems"]==2;assert "criteria" not in by["noul"]["required"]
