"""Dependency-free Jev client supporting OpenRouter Decisions and TypeSafe direct."""
...
def configure(*, provider: Any = None, base_url: Any = None, model: Any = None,
              typesafe_model: Any = None, timeout: Any = None) -> None:
    ...
    _configured_provider = raw_provider if raw_provider in {"openrouter", "typesafe"} else None
...
class JevClient:
    def __init__(
        ...
        *,
        ...
        if selected_provider not in {"openrouter", "typesafe"}:
            raise JevError("jev provider must be 'openrouter' or 'typesafe'")
        ...
        if selected_provider == "typesafe":
            ...
            missing = "TYPESAFE_API_KEY is not configured"
        else:
            ...
            missing = "OPENROUTER_API_KEY is not configured"
    ...
    def system_one(self, *, state: Any, questions: dict[str, dict[str, Any]], model: str | None = None) -> JevResponse:
        ...
        return JevResponse(
            ...
            provider=str(data.get("provider") or "TypeSafe"),
