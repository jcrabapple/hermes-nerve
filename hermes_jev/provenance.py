"""Canonical execution and receipt provenance for Hermes-Jev results."""
...
def classify_status(*, live_provider_call: bool, request_id: str = "", receipt_id: str = "", error: bool = False, stale: bool = False) -> str:
    """Return the reporting-safe provenance class for one result."""
    if error:
        return ERROR
    if stale:
        return UNVERIFIED
    if not live_provider_call:
        return LOCAL_ONLY
    if str(request_id or "").strip() or str(receipt_id or "").strip():
        return VERIFIED
    return UNVERIFIED
...
def execution_provenance(
    *,
    ...
    return {
        ...
        "provenance_status": classify_status(
...
def result_provenance(
    *,
    ...
    """Normalized provenance block intended for model-facing results and audits."""
    return {
        ...
        "subject_sha256": str(subject_sha256 or ""),
...
def attach(payload: dict[str, Any], *, live_provider_call: bool, transport: str = "openrouter-decisions") -> dict[str, Any]:
    ...
    out.setdefault("provenance_status", out["execution"].get("provenance_status", UNVERIFIED))
    return out
