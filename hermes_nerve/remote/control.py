...
def default_remote_control_path(host:RemoteHost,job_id:str)->str:
 root=host.control_root or ((host.workspace_root.rstrip("/")+"/.hermes-jev-control") if host.workspace_root else "/tmp/hermes-jev-control")
...
def read_local_control()->dict[str,Any]|None:
 path=str(os.getenv("HERMES_JEV_REMOTE_CONTROL_FILE") or "").strip()
...
def pre_tool_call(tool_name:str,args:dict,**kwargs):
 ...
 if tool_name in {"jev_work_event","jev_work_status","read_file","search_files"}:return None
 ...
 return {"action":"block","message":f"Hermes-Jev remote control {value} is active. Preserve useful work, emit jev_work_event CHECKPOINT_READY with plan/tests/artifacts, and stop new implementation work.","rule_key":"jev:remote-work-control"}
