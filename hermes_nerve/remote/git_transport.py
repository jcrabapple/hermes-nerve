...
class RemoteGitTransport:
    ...
    def _run_root(self, host: RemoteHost) -> PurePosixPath:
        root = host.git_cache_root or (
            str(PurePosixPath(host.workspace_root) / ".hermes-nerve-git")
            if host.workspace_root else "/tmp/hermes-nerve-git"
        )
    ...
    def collect(self, host: RemoteHost, prepared: PreparedRemoteGit) -> dict[str, Any]:
        ...
        cmd = "; ".join([
            ...
            f"cd -- {_q(remote_repo)}",
            "git add -A",
            "if ! git diff --cached --quiet; then git -c user.name='Nerve Remote Worker' -c user.email='hermes-nerve@local' commit -q -m 'Nerve remote worker result'; fi",
        ...
        with tempfile.NamedTemporaryFile(prefix="hermes-nerve-result-", suffix=".bundle", delete=False) as h:
