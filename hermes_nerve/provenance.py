...
def execution_provenance(*,live_provider_call:bool,transport:str="openrouter-decisions",request_id:str="",receipt_id:str="",error:bool=False,stale:bool=False)->dict[str,Any]:
 return {"engine":"nerve","version":VERSION,"transport":transport,"live_provider_call":bool(live_provider_call),"provenance_status":classify_status(live_provider_call=live_provider_call,request_id=request_id,receipt_id=receipt_id,error=error,stale=stale)}
def result_provenance(*,live_provider_call:bool,request_id:str="",receipt_id:str="",provider:str="",transport:str="openrouter-decisions",model:str="",contract:str="",created_at:str="",subject_sha256:str="",result_sha256:str="",error:bool=False,stale:bool=False)->dict[str,Any]:
 return {"verified_by":"nerve","provenance_status":classify_status(live_provider_call=live_provider_call,request_id=request_id,receipt_id=receipt_id,error=error,stale=stale),"request_id":str(request_id or ""),"receipt_id":str(receipt_id or ""),"provider":str(provider or ""),"transport":str(transport or ""),"model":str(model or ""),"contract":str(contract or ""),"created_at":str(created_at or ""),"subject_sha256":str(subject_sha256 or ""),"result_sha256":str(result_sha256 or ""),"stale":bool(stale)}
